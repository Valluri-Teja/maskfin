...
- **Rate limiting is in-memory, per-session, single-instance only**
  ...
  would be a shared store (Redis) for rate limit state, not built here
  since this runs as a single free-tier instance.
## "Offline" vs. the live demo - what that actually means

"Offline" refers to the detection and redaction pipeline: OCR and
regex/Luhn matching run entirely locally, with zero calls to any
third-party API. The only thing that ever leaves the machine is
already-redacted text, sent to Groq for the optional chat feature -
raw PII is never transmitted to an LLM.

The public demo at maskfin.onrender.com is a separate, deliberate
convenience: uploaded files transit to Render's server to run that
same local pipeline there, the same way any hosted app works. The
privacy guarantee this project is actually about is what touches an
